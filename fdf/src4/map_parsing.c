...
/*   Updated: 2025/01/22 12:39:36 by vlopatin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	parse_row(t_map *map, char *arr, int *c)
{
	...
	while(arr[j] && arr[j] != '\n')
	{
		map->points[*c].z = ft_atoi_base(&arr[j], 10) * map->z_scale;
		map->points[*c].x = ((*c) % map->width) * x_interval - map->width / 2 * x_interval;
		map->points[*c].y = ((*c) / map->width) * y_interval - map->height / 2 * y_interval;
